...
/* This file is previous re-hosting tool
 * so far not working in my test
 * Xingzhong
 */
public class RehostingUserInterface extends JDialog implements ActionListener {
	...
	public RehostingUserInterface(JFrame frame, SSP_Main parent) {
		...
		NVIDIA_GeForce_450 = new Checkbox("GeForce 450", HardwarePlatformGroup,
				true);
		NVIDIA_GeForce_280 = new Checkbox("GeForce 280", HardwarePlatformGroup,
				false);
		...
		Floating_Point = new Checkbox("Floating Point",
				FloatingFixedPointGroup, false);
		...
		WordlengthFractionlengthDisplayBlank2
				.add(new JLabel("Fraction Length:"));
		WordlengthFractionlengthDisplayBlank2.add(Box.createGlue());
		WordlengthFractionlengthDisplay
				.add(WordlengthFractionlengthDisplayBlank1);
		WordlengthFractionlengthDisplay.add(wordLength);
		WordlengthFractionlengthDisplay
				.add(WordlengthFractionlengthDisplayBlank2);
		...
		public void adjustmentValueChanged(AdjustmentEvent e) {
			...
			while (!TempCode.isEmpty()) {
				if (i > e.getValue()) {
					if (TempCode.indexOf("\n") > -1) {
						Output = Output
								+ TempCode.substring(0,
										TempCode.indexOf("\n") + 1);
						TempCode = TempCode
								.substring(TempCode.indexOf("\n") + 1);
					} else {
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if ((evt.getSource() == openSource)) {
			...
			FileDialog fd = new FileDialog(parent,
					"Please choose a source file for rehosting:",
					FileDialog.LOAD);
			fd.show();
			...
			if (selectedItem == null) {
				...
				selectedFlag = true;
				while ((selectedFlag && (fd.getFile().indexOf(".m") == -1))
						&& (selectedFlag
								&& (fd.getFile().indexOf(".c") == -1)
								&& (selectedFlag && (fd.getFile()
										.indexOf(".cc") == -1)) && (selectedFlag && (fd
								.getFile().indexOf(".vhd") == -1)))) {
					fd = new FileDialog(
							parent,
							"Wrong file selected. Please choose a source file for rehosting:",
							FileDialog.LOAD);
					fd.show();
				...
				if (selectedFlag) {
					...
					if ((selectedFlag && (fd.getFile().indexOf(".c") > -1))
							&& (fd.getFile().indexOf(".cc") == -1)) {
						MatlabFileOpen = false;
						CFileOpen = true;
						CPlusFileOpen = false;
						VHDLFileOpen = false;
					...
					/*
					 * String Temp = SourceMatlabCode; int Linelength = 0;
					 * while(! Temp.isEmpty()) { if (Temp.indexOf("\n") > -1) {
					 * Temp = Temp.substring(Temp.indexOf("\n") + 1); Linelength
					 * = Linelength + 1; } else Temp = ""; }
					 */
		...
		if ((evt.getSource() == saveSource)) {
			if (MatlabFileOpen) {
				Frame parent = new Frame();
				FileDialog fd = new FileDialog(parent,
						"Please choose a directory:", FileDialog.SAVE);
				fd.show();
				String selectedItem = fd.getFile();
				if (selectedItem == null) {
					// no file selected
		...
		if ((evt.getSource() == openXML)) {
			...
			Frame parent = new Frame();
			FileDialog fd = new FileDialog(parent,
					"Please choose an XML file for semantic inference:",
					FileDialog.LOAD);
			fd.show();
			String selectedItem = fd.getFile();
			boolean selectedFlag = false;
			if (selectedItem == null) {
				...
				while (selectedFlag && (fd.getFile().indexOf(".xml") == -1)) {
					fd = new FileDialog(
							parent,
							"Wrong file selected. Please choose an XML file for semantic inference:",
							FileDialog.LOAD);
					fd.show();
					selectedItem = fd.getFile();
					if (selectedItem == null) {
				...
				if (selectedItem == null) {
					...
					try {
						...
						if (Code.indexOf("function ") == -1)
							...
							CodeSaveTemp = Code.substring(0,
									Code.indexOf("function "));
							bufferedWriter.write(CodeSaveTemp);
							bufferedWriter.flush();
							bufferedWriter.close();
							Code = Code.substring(Code.indexOf("function "));
							CodeSaveTemp = Code.substring(Code
									.indexOf("function ") + 9);
							String FunctionFileName = "";
							while (CodeSaveTemp.indexOf("function ") > -1) {
								FunctionFileName = Code.substring(
										Code.indexOf("=") + 1,
										Code.indexOf("("));
								Code = CodeSaveTemp.substring(CodeSaveTemp
										.indexOf("function "));
								CodeSaveTemp = "function "
										+ CodeSaveTemp.substring(0,
												CodeSaveTemp
														.indexOf("function "));
								// System.out.print("fuck:" + CodeSaveTemp +
								// "\n");
								ffile = new File(fd.getDirectory()
										+ File.separator + FunctionFileName
										+ ".m");
								bufferedWriter = new BufferedWriter(
										new FileWriter(ffile));
								bufferedWriter.write(CodeSaveTemp);
								bufferedWriter.flush();
								bufferedWriter.close();
								CodeSaveTemp = Code.substring(Code
				...
				if (Temp.indexOf("\n") > -1) {
					...
					try {
						p.getAppletContext().showDocument(
								new URL("javascript:getSourceCode(\""
										+ TempOutput + "\")"));
		...
		if (evt.getSource() == toMatlab) {
			...
			try {
				p.getAppletContext().showDocument(
						new URL("javascript:getFloatingFixedSelection(\""
								+ fixedPointSelectionTemp + "\")"));
			} catch (MalformedURLException me) {
			}
			...
			if (wordLength.getText().isEmpty())
				...
				wordLengthTemp = Integer.parseInt(wordLength.getText()
						.toString());
			try {
				p.getAppletContext().showDocument(
						new URL("javascript:getFixedWordLength(\""
								+ wordLengthTemp + "\")"));
			} catch (MalformedURLException me) {
			}
			...
			if (fractionLength.getText().isEmpty())
				...
				fractionLengthTemp = Integer.parseInt(fractionLength.getText()
						.toString());
			try {
				p.getAppletContext().showDocument(
						new URL("javascript:getFixedFractionLength(\""
								+ fractionLengthTemp + "\")"));
			} catch (MalformedURLException me) {
			}
		...
		if (evt.getSource() == toCUDA) {
			...
			while (!Temp.isEmpty()) {
				if (Temp.indexOf("\n") > -1) {
					...
					try {
						p.getAppletContext().showDocument(
								new URL("javascript:getXMLHardwareCode(\""
										+ TempOutput + "\")"));
					} catch (MalformedURLException me) {
					}
					Temp = Temp.substring(Temp.indexOf("\n") + 1);
		...
		if (evt.getSource() == toOpenCL) {
			...
			/*
			 * String CLBMHardwareModelCode = ""; if
			 * (HardwarePlatformGroup.getSelectedCheckbox() ==
			 * NVIDIA_GeForce_450) { p.CLBMHardwareModelCode =
			 * "<Place name = \"GeForce_GTS_450\">\n    <Thing name = \"grid\">\n        <Place name = \"dimensionality\">\n            <Thing>3</Thing>\n            <Action>SET</Action>\n        </Place>\n        <Place name = \"dimensions\">\n            <Thing>\n                <Place name = \"x\">\n                    <Thing>65535</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"y\">\n                    <Thing>65535</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"z\">\n                    <Thing>65535</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"size\">\n                    <Thing>3</Thing>\n                    <Action>SET</Action>\n                </Place>\n            </Thing>\n            <Action>SET</Action>\n        </Place>\n    </Thing>\n    <Thing name = \"block\">\n        <Place name = \"dimensionality\">\n            <Thing>3</Thing>\n            <Action>SET</Action>\n        </Place>\n        <Place name = \"dimensions\">\n            <Thing>\n                <Place name = \"x\">\n                    <Thing>1024</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"y\">\n                    <Thing>1024</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"z\">\n                    <Thing>64</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"size\">\n                    <Thing>3</Thing>\n                    <Action>SET</Action>\n                </Place>\n            </Thing>\n            <Action>SET</Action>\n        </Place>\n    </Thing>\n    <Thing name = \"thread\">1024</Thing>\n    <Thing name = \"sharedMemory\">48KB</Thing>\n    <Thing name = \"localMemory\">512KB</Thing>\n    <Thing name = \"ConstantMemory\">64KB</Thing>\n</Place>\n"
			 * ; } if (HardwarePlatformGroup.getSelectedCheckbox() ==
			 * NVIDIA_GeForce_280) { p.CLBMHardwareModelCode =
			 * "<Place name = \"GeForce_GTX_280\">\n    <Thing name = \"grid\">\n        <Place name = \"dimensionality\">\n            <Thing>2</Thing>\n            <Action>SET</Action>\n        </Place>\n        <Place name = \"dimensions\">\n            <Thing>\n                <Place name = \"x\">\n                    <Thing>65535</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"y\">\n                    <Thing>65535</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"size\">\n                    <Thing>2</Thing>\n                    <Action>SET</Action>\n                </Place>\n            </Thing>\n            <Action>SET</Action>\n        </Place>\n    </Thing>\n    <Thing name = \"block\">\n        <Place name = \"dimensionality\">\n            <Thing>3</Thing>\n            <Action>SET</Action>\n        </Place>\n        <Place name = \"dimensions\">\n            <Thing>\n                <Place name = \"x\">\n                    <Thing>512</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"y\">\n                    <Thing>512</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"z\">\n                    <Thing>64</Thing>\n                    <Action>SET</Action>\n                </Place>\n                <Place name = \"size\">\n                    <Thing>3</Thing>\n                    <Action>SET</Action>\n                </Place>\n            </Thing>\n            <Action>SET</Action>\n        </Place>\n    </Thing>\n    <Thing name = \"thread\">512</Thing>\n    <Thing name = \"sharedMemory\">16KB</Thing>\n    <Thing name = \"localMemory\">16KB</Thing>\n    <Thing name = \"ConstantMemory\">64KB</Thing>\n</Place>\n"
			 * ; } String Temp = p.CLBMHardwareModelCode; String TempOutput =
			 * ""; while(Temp.indexOf("\"") > -1) { TempOutput =
			 * Temp.substring(0,Temp.indexOf("\"")); TempOutput = TempOutput +
			 * " CLBM_QOUTE "; Temp = TempOutput +
			 * Temp.substring(Temp.indexOf("\"") + 1); } TempOutput = "";
			 * while(! Temp.isEmpty()) { if (Temp.indexOf("\n") > -1) {
			 * TempOutput = Temp.substring(0,Temp.indexOf("\n")); try {
			 * p.getAppletContext().showDocument (new
			 * URL("javascript:getXMLHardwareCode(\"" + TempOutput +"\")")); }
			 * catch (MalformedURLException me) { } Temp =
			 * Temp.substring(Temp.indexOf("\n") + 1); } else { TempOutput =
			 * Temp; try { p.getAppletContext().showDocument (new
			 * URL("javascript:getXMLHardwareCode(\"" + TempOutput +"\")")); }
			 * catch (MalformedURLException me) { } Temp = ""; } }
			 */
